...
logging.basicConfig(filename="./p2.log", format='%(message)s',
                    level=logging.INFO, filemode='w')
...
def mapper():
    ...
    ifog = headers.index("fog")
    irain = headers.index("rain")
    ...
    for line in sys.stdin:
        ...
        key = format_key(float(splits[ifog]), float(splits[irain]))
        print key + "\t" + splits[ientries]
