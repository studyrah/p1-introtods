...
def entries_histogram(turnstile_weather):
    ...
    turnstile_weather['ENTRIESn_hourly'][turnstile_weather['rain'] == 0].hist(alpha=1.0, range=(0,6000), bins = 25)
    #turnstile_weather['...'] # your code here to plot a historgram for hourly 
    #entries when it is not raining    
    turnstile_weather['ENTRIESn_hourly'][turnstile_weather['rain'] == 1].hist(alpha=1.0, range=(0,6000), bins = 25)
...
#Here we had a null hypothesis that the mean of the two sets is equal but we 
#received a p value of ~ 0.02, less than the typical p critical of 0.05 so we 
#reject the null hypothesis and assert there is a statistically significant 
#difference between the mean of the two sets.  Non rainy days had the higher 
#mean so we expect that more people (on average) go through the turnstiles on 
#non rainy days.